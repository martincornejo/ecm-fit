...
focv = calc_pocv(data) # OCV curve lookup-table (function)
...
mean(abs2, v_lbfgs - df_train.v) # RMSE
...
begin
    ...
    lines!(ax[1], df_train.t, v_lbfgs)
    lines!(ax[1], df_train.t, v_eca)
    # lines!(ax[1], df_train.t, v_pso)
    lines!(ax[2], df_train.t, v_lbfgs - df_train.v)
